...
def extract_and_add_gufi_features(_df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _split_gufi(x: pd.DataFrame) -> pd.Series:
        ...
        gufi_timestamp_until_etd: int = int((gufi_flight_date - x.timestamp).seconds / 60)
        return pd.Series([gufi_flight_major_carrier, gufi_flight_destination_airport, gufi_timestamp_until_etd])

    _df[
        ["gufi_flight_major_carrier", "gufi_flight_destination_airport", "gufi_timestamp_until_etd"]
