    ...
    for d in DATA_GROUPS:
        ...
        for airport in AIRPORTS:
            ...
            current_airport = current_airport[
                (current_airport["timestamp"] >= start_time) & (current_airport["timestamp"] <= end_time)
            ]

            if (d == "tfm_estimated_runway_arrival_time") or (d == "tbfm_scheduled_runway_arrival_time"):
                current_airport["timestamp"] = current_airport["timestamp"].dt.ceil("15min")
                current_airport.sort_values("timestamp", inplace=True)
                current_airport = current_airport.groupby(["gufi", "timestamp"]).last().reset_index()
...
def CrossJoinDatesAirports(airports, start_time, end_time):
    ...
    for airport in airports:
        master_table = pd.read_csv(
            DATA_DIR / airport / f"prescreened_train_labels_{airport}.csv.bz2", parse_dates=["timestamp"]
        ).sort_values("gufi")
...
def ExtractAirportconfigFeatures(master_table, raw_data):
    ...
    for airport in AIRPORTS:
        ...
        current = current.set_index("timestamp").config.resample("15min").ffill().reset_index()
        ...
        current["feat_1_cat_hourofday"] = "hour_" + current["timestamp"].dt.hour.astype(int).astype(str)
        ...
        current["feat_1_isholiday"] = current["timestamp"].apply(lambda x: str(x)[:10] in holidays).astype(int)
        ...
        current["feat_1_active_departurerunways"] = current["feat_1_cat_airportconfig"].apply(
            lambda x: len(str(x).replace("_A_", "|").replace("D_", "").split("|")[0].split("_"))
        )
        current["feat_1_active_arrivalrunways"] = current["feat_1_cat_airportconfig"].apply(
            lambda x: len(str(x).replace("_A_", "|").replace("D_", "").split("|")[1].split("_"))
        )

        # Indicate the angle of the configuration
        numbers = current["feat_1_cat_airportconfig"].apply(
            lambda x: [int(s) for s in x.replace("L", "").replace("R", "").split("_") if s.isdigit()]
        )

        current["feat_1_max_directions"] = numbers.apply(lambda x: max(x) if len(x) > 0 else 0)
        current["feat_1_min_directions"] = numbers.apply(lambda x: min(x) if len(x) > 0 else 0)
        current["feat_1_unique_directions"] = numbers.apply(lambda x: len(set(x)) if len(x) > 0 else 0)
        ...
        for i in [4, 8, 12, 16, 20, 24]:
            ...
            current[f"feat_1_active_arrival_roll_{i}"] = current[f"feat_1_active_arrivalrunways"].rolling(i).mean()
        ...
        current.loc[
            (airport + ":" + current["feat_1_cat_airconfigother"]).isin(CLASSES) == False,
        ...
        current["feat_1_config_length"] = current["feat_1_cat_airportconfig"].apply(lambda x: len(str(x)))
        current["feat_1_config_nelements"] = current["feat_1_cat_airportconfig"].apply(lambda x: len(str(x).split("_")))
        ...
        for i in [4, 8, 12, 16, 20, 24]:
            master_table[f"feat_1_{airport}_dep_roll_{i}"] = master_table["timestamp"].map(
                dict(zip(current["timestamp"], current[f"feat_1_active_dep_roll_{i}"]))
            )
            master_table[f"feat_1_{airport}_arr_roll_{i}"] = master_table["timestamp"].map(
                dict(zip(current["timestamp"], current[f"feat_1_active_arrival_roll_{i}"]))
            )
            master_table[f"feat_1_{airport}_nchanges_{i}"] = master_table["timestamp"].map(
...
def ExtractLampFeatures(master_table, raw_data):
    ...
    for feat in weather_feats:
        master_table[feat + "_global_min"] = master_table["timestamp"].map(weather.groupby("timestamp")[feat].min())
        master_table[feat + "_global_mean"] = master_table["timestamp"].map(weather.groupby("timestamp")[feat].mean())
        master_table[feat + "_global_max"] = master_table["timestamp"].map(weather.groupby("timestamp")[feat].max())
        master_table[feat + "_global_std"] = master_table["timestamp"].map(weather.groupby("timestamp")[feat].std())
...
def ExtractETDFeatures(master_table, raw_data):
    ...
    master_table = master_table.merge(etd_features, how="left", on=["airport", "timestamp"])
...
def ExtractERAFeatures(master_table, raw_data, column="estimated_runway_arrival_time"):
    ...
    for airport in master_table["airport"].unique():
        ...
        complete_era["time_ahead"] = (complete_era[column] - complete_era["timestamp"]).dt.total_seconds()
        complete_era = complete_era.groupby(["gufi", "timestamp"]).first().reset_index()

        for i in [30, 60, 90, 120, 150, 180, 210, 240, 270, 300, 330, 360]:
            complete_era[f"next_{i}min"] = (complete_era["time_ahead"] < i * 60).astype(int)
        ...
        era_aggregation.columns = [prefix + c if c != "timestamp" else c for c in era_aggregation.columns]
        era_aggregation["airport"] = airport
        era_features = pd.concat([era_features, era_aggregation])

    master_table = master_table.merge(era_features, how="left", on=["airport", "timestamp"])
...
def ExtractGufiTimestampFeatures(master_table, raw_data, group):
    ...
    for airport in master_table["airport"].unique():
        ...
        for intervals in [
            ...
            current[f"feat_{group}_count_{intervals}"] = current.rolling(intervals).gufi.count()
    ...
    master_table = master_table.merge(gufi_features, how="left", on=["airport", "timestamp"])
...
def AddTargets(df):
    ...
    for look in LOOKAHEADS:
        df[f"target_{look}"] = df.groupby("airport").feat_1_cat_airportconfig.shift(-int(look / 15))
        df[f"target_{look}"] = df["airport"] + ":" + df[f"target_{look}"]
        df.loc[df[f"target_{look}"].isin(CLASSES) == False, f"target_{look}"] = df["airport"] + ":other"
