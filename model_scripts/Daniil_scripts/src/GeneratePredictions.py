...
def GeneratePredictions(to_submit: pd.DataFrame, models_path: str, master_table: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    for airport in to_submit["airport"].unique():
        ...
        for l in LOOKAHEADS:
            ...
            current_predictions = (
                current_predictions.set_index(["airport", "timestamp", "lookahead"]).stack().reset_index()
            )
