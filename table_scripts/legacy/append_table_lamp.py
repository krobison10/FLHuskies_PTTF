...
if __name__ == "__main__":
    ...
    table: pd.DataFrame = pd.read_csv(
        DATA_DIR / airport / f"train_labels_{airport}.csv.bz2", parse_dates=["timestamp"]
    ...
    feature_tables: dict[str, pd.DataFrame] = {
        ...
        "lamp": pd.read_csv(
            DATA_DIR / airport / f"{airport}_lamp.csv{ext}", parse_dates=["forecast_timestamp", "timestamp"]
        ),
    }
