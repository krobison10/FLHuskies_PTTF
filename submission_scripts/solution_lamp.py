...
def predict(
    ...
    encoded_columns = ["aircraft_engine_class", 
                       "aircraft_type",
                       "major_carrier",
                       "flight_type",
                       "cloud",
    ...
    for airport in table.airport.unique():
        prediction.loc[table.airport == airport, "minutes_until_pushback"] = model[airport].predict(table.loc[table.airport == airport, features].to_numpy())
